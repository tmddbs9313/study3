...
public class FinalFieldTest {
    ...
    *  : final 은 종단의 의미를 가지는 키워드이다.
        ...
        5. non-static 메소드 : 메소드 재작성(overloading) 불가
        6. static 메소드 : 메소드 재작성(overriding) 불가
        7. 클래스 : 상속 불가  */

    /*목차. 1. non-static field 에 final 사용*/

    /*필기. final 은 변경 불가의 의미를 가진다.
    *  따라서 초기 인스턴스가 생성되고 나면 기본값 0 이 필드에 들어가게 되는데, 그 초기화 이후 값을 변경할 수 없기 때문에
    *  선언하면서 바로 초기화 (값 넣기)를 해주어야 한다.그렇지 않으면 compile error 가 발생.*/
...
//    private final int nonstaticNum;
    ...
    public FinalFieldTest(String nonStaticNme) {this. NON_STATIC_NAME =nonStaticNme;}

    /*목차. 2. static field 에 final 사용.*/

    /*필기. static 에서도 자바에서 지정한 기본값이 초기에 들어가기 때문에 final 키워드 사용시 초기화 하지 않으면 에러가 발생한다*/

    //private static final int STATIC_NUM;

    private static final int STATIC_NUM = 1;

    /*필기. 생성자를 이용한 초기화는 불가능ㅎ다ㅏ.
    *  생성자는 인스턴스가 생성되는 시점에 호출되기 떄문에 그 전에는 초기화가 일어나지 못한다. 하지만 static 은 프로그램이 start
    *  될때 할당되기 때문에 초기화가 되지않은 상태로 선언된 것과 동일하게 기본값으로 초기화 된 후 값을 변경하지 못하기 때문에 에러가 발생한다*/

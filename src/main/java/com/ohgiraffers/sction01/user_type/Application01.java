...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        /*수업목표. 클래스가 무엇인지 이해하고 작성 할 수 있다.*/

        /*필기. 회원정보를 관리하기 위해 회원의 여러 정보 (아이디, 비밀번호, 이름, 나이, 성별, 취미)를 관리*/
        ...
        int age = 22;
        char gender = '여';
        String[] hobby = new String[] {"축구", "배드민턴", "탁구"};
        ...
        /*필기. 이렇게 각각의 변수로 관리하게 되면 여러가지 단점이 있다.
        *  1. 변수명을 다 관리해야하는 어려움이 생긴다
        *  2. 모든 회원 정보를 인자로 메소드 호출 시 값을 전달해야 하면 너무 많은 값들을 인자로 전달해야 해서 한 눈에 안들어온다
        *  3. 리턴은 1개의 값만 가능하기 떄문에 회원정보를 묶어서 리턴값으로 사용할 수 없다
        *  (서로 다른 자료형들이기 때문이다.)*/

        /*목차. 2. 사용자 정의의 자료형 사용하기*/
        /*목차. 2-1. 변수 선언 및 객체 생성*/

        /*필기. 자료형 변수명 = new 클래스명();  <- 지금까지 사용한 이 구문은 객체(instance)를 생성하는 구문이다.
        ...
        *  */
        ...
        /*필기. 이렇게 위에처럼 객체를 생성하고 나면, 서로 다른 자료형들은 하나의 member 라는 변수명으로
        *  관리 할 수 있도록 공간을 생성한 것이다.
        *  heap 이라는 공간에 생성되기 때문에 jvm 이 제공하는 기본값들로 초기화된다.*/

        /*필기. 필드에 접근하기 위해서는 래퍼런스변수명 (주소값을 보관하고 있는 변수명). 필드명 <- 이렇게 접근한다.
        *  '.' 참조 연산자 라고 하는데, 래퍼런스 변수가 참조하고 있는 주소로 접근한다라는 의미
        *  각 공간은 필드명으로 접근한다.*/
        ...
        member.age = 22;
        member.gender = '남';
        member.hobby = new String[] {"야구", "볼링" ,"테니스"};
